...
Sub DrawScreen
  ...
  Colour RGB(cyan)
  Font 7
  y = MM.VRes - MM.Info(FONTHEIGHT) * 14.6
...
Sub GetDifficulty
  ...
  Box (MM.HRes/3) * 2 + 3, MM.Info(FONTHEIGHT) * 15, MM.HRes, MM.VRes, 1, BackColour, BackColour
  Text LeftX, 5, "Blocks", c, TitleF, TitleFS, RGB(white), BackColour
  ...
  Do
    Select Case get_input%()
      ...
      Case ctrl.A, ctrl.START
        Colour RGB(cyan)
        Font 7
        Text RightX, MM.Info(FONTHEIGHT) * 24.6, "START  = PAUSE", c
        Text RightX, MM.Info(FONTHEIGHT) * 25.6, "SELECT = QUIT ", c
        Font GeneralF, GeneralFS
        Colour RGB(white)
