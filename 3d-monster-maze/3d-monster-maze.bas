...
If Mm.Device$ = "MMBasic for Windows" Then Mode -7 Else Mode 7
Cls
gamemite.set_title("3D Monster Maze", "Malcolm Evans & Martin Herhaus", VERSION)

Const FRAME_DURATION = 120
Const STATE_INTRO = 0, STATE_GAME = 1, STATE_WIN = 2, STATE_LOSE = 3

Dim a As Integer
Dim ctrl$ = "wii_classic_3"
...
' Try initialisating the controller, if it fails default to no controller.
ctrl.init_keys()
On Error Ignore
Call ctrl$, ctrl.OPEN
...
Function get_input%(ignore%)
  ...
  Select Case get_input%
    Case ctrl.START
      If state% = STATE_GAME Then on_quit()
