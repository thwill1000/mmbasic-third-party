...
' text is not modified or deleted but feel free to change or add Caves
...
'!elif defined(GAMEMITE)
  '!replace { Option Simulate "PicoMiteVGA" } { Option Simulate "Game*Mite" }
...
Const FILE_ROOT$ = Choice(Mm.Info(Path) = "NONE", "", Mm.Info(Path))
Const STATE_SHOW_TITLE = 0, STATE_PLAY_GAME = 1, STATE_LEVEL_DONE = 2

Dim key%
Dim Lev$(33)Length 64,spr%(40),ro$,mite$=""
...
game.init_window("Gems'N'Rocks", VERSION)
...
Do
  ...
  key% = get_input%()
  Move_PL
  If state% = STATE_LEVEL_DONE Then Exit Do
...
If state% = STATE_LEVEL_DONE Then Goto leveldone
...
Function get_input%(ignore%)
  ...
  If Not get_input% Then If ctrl.keydown%(109) Then get_input% = ctrl.B ' m = Mute
  ...
  Select Case get_input%
    ...
    Case ctrl.B
      If state% = STATE_PLAY_GAME Then
        mus = Not(mus)
        toggle_snd()
        Do While get_input%(1) : Pause 5 : Loop ' Wait for key release.
      EndIf
  End Select
End Function
...
'Intermediate title
    ...
    CLS
    If  mite$="MMB4W" Then
        Load bmp FILE_ROOT$ + "images\menuscreen.bmp"
    Else
        Load image FILE_ROOT$ + "images\menuscreen.bmp"
    EndIf
    If mite$="Game*Mite" Then FRAMEBUFFER copy f,n

    Local t% = Timer + 1000
    Do While Timer < t%
      If get_input%() And (ctrl.START Or ctrl.A) Then Exit Sub
      Pause 10
    Loop
  ...
  Do
    ...
    If f=800 Then
      ...
      If mite$="Game*Mite" Then FRAMEBUFFER copy f,n
    EndIf
  Loop Until get_input%() And (ctrl.A Or ctrl.START)
End Sub
...
Sub toggle_snd
  Play Stop
  Play modfile FILE_ROOT$ + Choice(mus, "sounds/gems-n-rocks.mod", "sounds/silence.mod")
End Sub
...
'-----------------------------------
  ...
  Select Case bl
    ...
    'move rock if there is space behind it
          ...
          Case ex:If exi Then state% = STATE_LEVEL_DONE
  End Select
'-----------------------------------
     Case ctrl.RIGHT
        spl=12
...
Sub showscreen x As Integer, y As Integer, xo As Integer
...
Sub intro
  ...
  Cls
  If mite$="MMB4W" Then
    Load bmp FILE_ROOT$ + "images\title.bmp",0,-13
  Else
    Load image FILE_ROOT$ + "images\title.bmp",0,-13
  EndIf
  If mite$="Game*Mite" Then FRAMEBUFFER copy f,n
  Local t% = Timer + 1000
  Do While Timer < t%
  ...
  ' Flash the START or SPACE instruction.
  Do
    If f% = 400 Then Text 0,232," START OR SPACE "
    If f% = 800 Then Text 0,232," START OR SPACE ",,,,0,col(7) : f% = 0
    If mite$="Game*Mite" Then FRAMEBUFFER copy f,n
    If get_input%() And (ctrl.START Or ctrl.A) Then Exit Sub
    Pause 1
    Inc f%
...
Sub on_quit()
  If state% <> STATE_SHOW_TITLE Then Const old_music% = mus : mus = 0 : toggle_snd()
  msgbox.beep(1)
...
Data " *PRESS START OR SPACE TO LET THE FUN BEGIN!    ",""
...
Data " PRESS START OR SPACE TO TRY AGAIN!            ",""
colors:
